<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        if ($project->isUserProjectMember($id_project, $_SESSION["user_id"])) {
            $stmt = $pdo->query("SELECT sender_id, message, sent_at FROM chat_messages WHERE project_id = $id_project ORDER BY id DESC LIMIT 40");
            ...
            foreach (array_reverse($messages) as $msg) {
                ...
                $style = ($_SESSION['user_id'] == $msg['sender_id']) ? ' style="color:green;word-break: break-word;"' : 'style="word-break: break-word;"';
